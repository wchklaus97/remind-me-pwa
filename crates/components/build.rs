...
//! `crates/components/assets/` before compilation.
...
fn main() {
    ...
    // Calculate workspace root: crates/components -> crates -> workspace root
    let workspace_root = crate_path
        .parent()  // crates/components -> crates
    ...
    // Copy assets directory (only if destination doesn't exist or is outdated)
    // For simplicity, we always copy to ensure assets are up to date
